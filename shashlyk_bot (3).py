...
def new_order(chat_id):
    user_carts[chat_id] = {}
    user_state[chat_id] = "choosing"
    waiter_bot.send_message(chat_id, "🔥 *Выберите позиции:*",
                            parse_mode="Markdown", reply_markup=menu_keyboard({}))

@waiter_bot.message_handler(commands=["start"])
def cmd_start(msg):
    waiter_bot.send_message(msg.chat.id,
        "👋 Добро пожаловать!\nНажмите кнопку ниже чтобы создать заказ.",
        reply_markup=main_keyboard())

@waiter_bot.message_handler(func=lambda m: m.text in ["🔥 Новый заказ", "/new"])
def cmd_new(msg):
    new_order(msg.chat.id)
...
def process_table(msg):
    ...
    waiter_bot.send_message(msg.chat.id,
        f"✅ Заказ *#{oid}* отправлен шашлычнику!",
